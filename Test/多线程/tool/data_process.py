...
class Xpath():
    ...
    def xpath(self, x=None, filter='style|script', character=True, is_list=True, easy=False, rule=None):
        if filter != None:
            ...
            obj = tree.xpath(x)
            obj = process_text(obj, character, is_list, rule)
...
# etree解析
def process_text(obj, character=True, is_list=False, rule=None):
    L = []
