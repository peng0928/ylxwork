...
def timechange(start_date):
    start_date = re.findall('\d{4}-\d{1,2}-\d{1,2}', start_date.replace('/', '-'))[0]
    middle = datetime.datetime.strptime(start_date, '%Y-%m-%d')
    end_date = datetime.datetime.strftime(middle, '%Y%m%d')
    return end_date

def timechange2(start_date):
    start_date = re.findall('\d{4}-\d{1,2}-\d{1,2}', start_date.replace('/', '-'))[0]
    middle = datetime.datetime.strptime(start_date, '%Y-%m-%d')
    end_date = datetime.datetime.strftime(middle, '%Y-%m-%d')
    return end_date
