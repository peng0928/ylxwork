...
class Xpath():
    ...
    def dpath(self, x=None, rule=None, Format=True):
        x = x.split('|')
        x = [i + '//text()' if '/@' not in i else i for i in x]
        x = '|'.join(x)
        ...
        obj = self.res.xpath(x)
        obj = ''.join(obj)
        obj = process_date(data=obj, rule=rule, Format=Format)
        return obj
...
# 取出时间 data->时间戳，文本
def process_date(data=None, rule=None, Format=True):
    data = data
    if len(data) == 0:
        return None
    if len(data) == 13:
    ...
    if len(data) == 10 and '-' not in data:
        ...
        if len(result) == 0:
            ...
            if Format is False:
                ...
                result = ' '.join(result[0])
                result = re.findall('\d+-\d{1,2}-\d{1,2}', result)[0]
                return result
