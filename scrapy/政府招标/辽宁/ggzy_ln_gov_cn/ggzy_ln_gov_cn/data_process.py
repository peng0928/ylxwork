...
class Xpath():
    ...
    def xpath(self, x=None, p='', filter=None):
        le = x.split('|')
        ...
        try:
            if filename is not None and filelink is not None:
                for i in range(len(filelink)):
                    is_file = bool(re.search(r'(\.zip|\.pdf|\.png|\.doc|\.txt|\.ppt|\.html|\.xls|\.rar)', str(filename[i])))
                    is_link = bool(re.search(r'(\.zip|\.pdf|\.png|\.doc|\.txt|\.ppt|\.html|\.xls|\.rar)', str(filelink[i])))
...
def get_link(url=None, rpath=None):
